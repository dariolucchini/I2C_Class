...
ErrorCode I2C_Peripheral_ReadRegister(uint8_t device_address, 
    ...
    if (error == I2C_Master_MSTR_NO_ERROR)
    {
        ...
        if (error == I2C_Master_MSTR_NO_ERROR)
        {
            // Send a restart condition solo dispositivo con address giusto risponde
    ...
    //Return
    return error ? ERROR : NO_ERROR; //if else tutto attaccato
...
ErrorCode I2C_Peripheral_WriteRegister(uint8_t device_address, // error code è quello che butta fuori: 
                                       uint8_t register_address, //numero di errore (0 no error, 1 errore)
                                       uint8_t data) {
